...
import {
  Brain, MessageSquare, ArrowRight, Shield, CheckCircle2,
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background grid-bg">
      ...
      <nav className="border-b border-border/50 backdrop-blur-sm sticky top-0 z-50 bg-background/80">
        <div className="container flex items-center justify-between h-16">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="outline" className="text-xs border-primary/30 text-primary hidden sm:inline-flex">Active</Badge>
          </div>
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            {isAuthenticated && user?.role === "admin" && (
              <>
                ...
                <Link href="/research"><Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground">Research</Button></Link>
          ...
          <div className="flex md:hidden items-center gap-2">
            {!isAuthenticated ? (
              <a href={getLoginUrl()}>
                <Button size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground text-xs px-3">Sign In</Button>
              </a>
            ) : (
              <div className="w-7 h-7 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-xs font-medium text-primary">
                {user?.name?.[0]?.toUpperCase() ?? "U"}
            ...
            <button
              className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
          ...
          <div className="md:hidden border-t border-border/50 bg-background/95 backdrop-blur-sm">
            <div className="container py-4 space-y-1">
              <Link href="/chat" onClick={() => setMobileMenuOpen(false)}>
                <div className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-muted/50 transition-colors text-sm text-muted-foreground hover:text-foreground">
                  <MessageSquare className="w-4 h-4" /> Chat with Unstor
                </div>
              </Link>
              {isAuthenticated && user?.role === "admin" && (
                <>
                  ...
                  <Link href="/knowledge" onClick={() => setMobileMenuOpen(false)}>
                    <div className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-muted/50 transition-colors text-sm text-muted-foreground hover:text-foreground">
                      <Layers className="w-4 h-4" /> Knowledge
                    </div>
                  </Link>
                  ...
                  <Link href="/ifa" onClick={() => setMobileMenuOpen(false)}>
                    <div className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-muted/50 transition-colors text-sm text-muted-foreground hover:text-foreground">
                      <BookOpen className="w-4 h-4" /> Ifá Explorer
                    </div>
                  </Link>
                  <Link href="/owner-chat" onClick={() => setMobileMenuOpen(false)}>
                    <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-primary/10 text-primary text-sm font-medium">
                      <MessageSquare className="w-4 h-4" /> Owner Chat
                    </div>
                  </Link>
                </>
              )}
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div className="space-y-7">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <a href="#how-it-works">
                <Button size="lg" variant="outline" className="border-border gap-2 w-full sm:w-auto">
          ...
          <div className="relative rounded-2xl border border-border bg-card p-6 sm:p-8 overflow-hidden space-y-6">
            ...
            <div className="relative">
              ...
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 p-3 rounded-xl bg-muted/40">
      ...
      {/* ── 2. WHAT IT IS / IS NOT ── */}
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="text-center mb-10 sm:mb-12 space-y-3">
          <h2 className="text-2xl sm:text-3xl font-display font-bold">What Unstor Is — And What It Is Not</h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base">Clarity builds trust. Here is exactly what you are working with.</p>
        </div>
        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-6 sm:p-7 space-y-4">
      ...
      {/* ── 3. HOW IT WORKS ── */}
      <section id="how-it-works" className="container py-16 sm:py-20 border-t border-border/30">
        <div className="text-center mb-10 sm:mb-14 space-y-3">
          <h2 className="text-2xl sm:text-3xl font-display font-bold">How Unstor Guides You</h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base">A structured 4-step process from your situation to clear direction.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 max-w-5xl mx-auto">
          {[
            { step: "01", icon: MessageSquare, title: "Input", body: "You describe your situation, concern, or question in your own words." },
            { step: "02", icon: Layers, title: "Pattern Recognition", body: "Unstor analyses behavioural signals, emotional indicators, and symbolic parallels using Ifá structure." },
            { step: "03", icon: Eye, title: "Interpretation", body: "You receive structured reflection, pattern explanation, and perspective from both traditional and modern frameworks." },
            { step: "04", icon: Compass, title: "Direction", body: "Unstor suggests practical actions, mindset adjustments, and areas to observe or improve." },
          ].map(({ step, icon: Icon, title, body }) => (
            <div key={step} className="relative rounded-2xl border border-border bg-card p-5 sm:p-6 space-y-4 hover:border-primary/30 transition-all duration-200">
      ...
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="text-center mb-10 sm:mb-12 space-y-3">
          <h2 className="text-2xl sm:text-3xl font-display font-bold">Built on Structured Knowledge Systems</h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base">Four distinct frameworks, integrated into a single coherent intelligence.</p>
        </div>
        <div className="grid sm:grid-cols-2 gap-5 sm:gap-6 max-w-4xl mx-auto">
          {[
            { color: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/20", icon: BookOpen, title: "Ifá Symbolic Framework", body: "Pattern-based interpretation using Odù as reflective archetypes — not divination. Symbolic frameworks inspired by Ifá traditions." },
            { color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20", icon: Brain, title: "Behavioural & Psychological Insight", body: "Understanding habits, reactions, and decision patterns through evidence-based behavioural science and psychology." },
            { color: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20", icon: FlaskConical, title: "Traditional Wellness Knowledge", body: "Contextual knowledge from herbal and traditional systems — always supportive, never prescriptive. Practitioner guidance always recommended." },
            { color: "text-violet-400", bg: "bg-violet-500/10", border: "border-violet-500/20", icon: Library, title: "Research & Modern Knowledge", body: "Continuous ingestion of structured knowledge across science, philosophy, and human development. Evolving with new data." },
          ].map(({ color, bg, border, icon: Icon, title, body }) => (
            <div key={title} className={`rounded-2xl border ${border} ${bg} p-6 sm:p-7 space-y-4`}>
      ...
      {/* ── 5. SAFETY & TRUST ── */}
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="max-w-3xl mx-auto rounded-2xl border border-border bg-card p-7 sm:p-10 space-y-8">
          <div className="text-center space-y-3">
            ...
            <h2 className="text-xl sm:text-2xl font-display font-bold">Safety, Boundaries, and Responsibility</h2>
      ...
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="text-center mb-10 sm:mb-12 space-y-3">
          <h2 className="text-2xl sm:text-3xl font-display font-bold">What You Can Use Unstor For</h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base">Practical, grounded applications for real situations.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 max-w-4xl mx-auto">
          {[
            ...
            <div key={text} className="flex items-start gap-3 p-4 sm:p-5 rounded-xl border border-border bg-card hover:border-primary/30 hover:bg-accent/30 transition-all duration-200">
      ...
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="max-w-3xl mx-auto text-center space-y-7 sm:space-y-8">
          <div className="space-y-3">
            <h2 className="text-2xl sm:text-3xl font-display font-bold">Start Your Session With</h2>
            <p className="text-muted-foreground text-sm sm:text-base">These prompts are designed to open a structured, meaningful conversation.</p>
          </div>
          ...
          <Link href="/chat">
            <Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2 mt-2 w-full sm:w-auto">
      ...
      {/* ── 8. PRIVACY ── */}
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="max-w-2xl mx-auto rounded-2xl border border-border bg-card p-6 sm:p-8 space-y-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center">
      ...
      <section className="container py-16 sm:py-20 border-t border-border/30">
        <div className="max-w-3xl mx-auto text-center space-y-6">
          ...
          <blockquote className="text-lg sm:text-xl font-display font-medium leading-relaxed text-muted-foreground">
      ...
      <footer className="border-t border-border/30 py-8">
        <div className="container flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground text-center sm:text-left">
          ...
          <span>Reflective guidance only. Not medical advice. Not divination.</span>
